...
import type { Cwd, AbsFilePath } from "../utils/files";
...
function findSkillFilesInDirectory(
  ...
  try {
    ...
    let skillsDirPath: string;
    if (path.isAbsolute(expandedDir)) {
      skillsDirPath = expandedDir;
    } else if (context.cwd) {
      skillsDirPath = path.join(context.cwd, expandedDir);
    } else {
      context.logger.warn(
        `Skipping relative skills path "${skillsDir}" because no cwd is set`,
      );
      return skillFiles;
    }
...
export function formatSkillsIntroduction(skills: SkillsMap, cwd?: Cwd): string {
  ...
  const skillsList = Object.values(skills)
    .map((skill) => {
      ...
      const relPath = cwd ? path.relative(cwd, skill.skillFile) : undefined;
      const displayPath =
        relPath && !relPath.startsWith("..") ? relPath : skill.skillFile;
