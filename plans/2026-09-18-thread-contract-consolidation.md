...
- `TitleSupervisor` no longer carries `thread: Thread | undefined` plus a `requested` flag. Attachment and request state are one field: `{ type: "unattached" } | { type: "pending"; thread } | { type: "requested" }`, so "requested with no thread" is not representable and `onSubmission` needs no `!thread` guard. `attach` survives (the supervisor list is built before the thread exists) but now transitions that state.
- `ThreadCoreView` spells out `editedFileGroups: EditedFileGroup[]` and `latestUsage: Usage | undefined` instead of indexed-access types into `ThreadCore`, and the two corresponding getters carry explicit return annotations, so a core-side widening is caught at the republished view rather than flowing through it.
- Added coverage: `thread-supervisor.test.ts > SupervisorChain onSubmission` (a stale submission consults no member; a throwing member is logged and later members still run); `thread-assembly.test.ts > never requests a title for a compact thread` (the `threadType === "compact"` branch, driven by a real submission) and `> still requests a title after the core was replaced` (the per-core chain is rebuilt, and the supervisor — being a chat supervisor — is still consulted).
