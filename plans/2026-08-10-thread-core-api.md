...
3. **[DONE] Change the interfaces.** `AgentPhase`, `ThreadPhase`, `TurnActivity`, `SendResult`, `ThreadResult`, `QueuedMessage`, `onUpdate`, and the `AgentHooks` trio. Delete `ThreadMode`, `ThreadCoreEvents`, `TurnEndReason`, `ThreadCoreAction` and the public `state` bag. Fold `onBeforeToolResponse` into the runner's options, give `Runner.clone` a hooks argument, delete `bindHooks`. Extend `RequestAction` with `inject`. Decide the `phase.activity` stored-vs-getter question. `npx tsc -b` is the work queue, not a gate, from here until stage 6.

   Stage 3 notes (**[DONE]**):
   - **New module `node/core/src/thread-api.ts`** holds the new vocabulary: `YieldValue`, `TurnActivity`, `ThreadPhase`, `SendOptions`, `SendResult`, `ThreadResult`, `QueuedMessage`, `OnUpdate` and the `AgentHooks` trio (`onEndTurn` / `onYield` / `onBeforeRequest`). All are exported from `@magenta/core`.
   - **Name collision resolved by renaming the runner-level hooks type.** `AgentHooks` in `provider-types.ts` (the `executeTools` / `onUpdate` pair) became `RunnerHooks`, which frees `AgentHooks` for the `Agent` → `Thread` trio and matches the stage-1 `Runner` rename.
   - **Runner hook lifetime closed.** `onBeforeToolResponse` is now a field of `RunnerHooks` / `AgentOptions` rather than an assignable property on `Runner`; `bindHooks` is deleted and `clone(hooks: RunnerHooks)` takes its collaborators at creation. Consequently `AgentDeps.runnerInit` carries `{type: "cloned", cloneFrom, truncateTo}` — the `Agent` performs the clone and the truncation itself, so a cloned runner is never briefly pointing at its source's `executeTools`. `Thread.clone` passes the source runner and fork index down instead of pre-cloning. `ThreadInit.clone` carries `sourceRunner` + `nativeMessageIdx`.
   - **`HandoffAction` → `RequestAction`, `HandoffContext` → `RequestContext`, `ThreadSupervisor.onHandoff` → `onBeforeRequest`** (mechanical rename; the "fires before *every* request" semantics are stage 4). `RequestAction` gains the `inject` variant (`text`, opaque `annotation`, `alsoCompact`), and the merge in `Agent` joins injected texts and ORs `alsoCompact`. No supervisor returns `inject` yet; the end-turn call site handles it by queueing the text via `prependToNextTurn` as a placeholder, with the real per-request injection (and the annotation riding on the message) landing in stage 4.
   - **`phase.activity` is a read-through getter, not a stored mirror** — settled as the plan leaned: everything in `TurnActivity` (streaming block, retry countdown, active tools, `lastEventTime`) moves between renders, so a copy would be stale by construction. `Thread.phase` is a getter deriving `ThreadPhase` from `state.mode` + the runner's phase + `lastTurnResult`/`failedSubmit`. Deviation: `TurnActivity.running_tools` carries `{requested, truncated}` (the runner's own shape) rather than the plan's speculative `ReadonlyMap<ToolRequestId, ActiveToolEntry>`, since that is the data that actually exists to read through to.
   - **Deviation, deliberate: the deletions are deferred to stages 4/5.** `ThreadMode`, `AgentEvents`, `TurnEndReason`, `AgentAction` and the public `state` bag cannot be removed without the implementation rewrite that stage 4 performs and the consumer rewrite in stage 5 — every removal cascades into ~180 read sites. This stage therefore *adds* the new vocabulary (and a working `Thread.phase` projection over the old state) so stages 4–6 can migrate consumers onto it incrementally, and keeps `npx tsc -b`, biome and the suite green throughout.
   - New tests in `node/core/src/agent.test.ts` (`Thread.phase`): idle-with-no-result before a send, `running`/`streaming` during a turn then `idle` + `{type: "completed"}`, and a yield surfacing as `idle` with a `yielded` `lastResult`.
   - Suite: the same four nvim-backed files that flake under parallel load at HEAD (`script-manager`, `spawn-subagents`, `thread-abort`, `fork-thread`) fail in a whole-suite run and pass when run individually; `tsc -b` and `biome check` are clean.
   Stage 3 review follow-ups (addressed):
   - **`RequestAction` no longer represents compaction twice.** `inject` carried `alsoCompact?: boolean`, so a compact request was reachable from two variants — and the merge silently dropped the collected `nextPrompt`s whenever any supervisor injected. The variant now carries `andThen: {type:"compact"; nextPrompt} | {type:"none"}` (named `andThen` rather than the plan's `then`, which biome's `noThenProperty` forbids on object literals), and `requestedCompaction(action)` is the single accessor both call sites use, so no consumer tests two variants.
   - **The merge moved out of `Agent` into `mergeRequestActions(actions)` in `thread-supervisor.ts`** — it is arbitration policy, which the plan says belongs in the composer, and it is now unit-testable without a thread. `Agent.consultBeforeRequestSupervisors` just collects the answers and delegates. New tests in `thread-supervisor.test.ts`: a compact request survives alongside an injection, injected texts join in order, compact prompts join, and an all-`none` list merges to `none`.
   - **`inject` is covered at its consumer.** New `agent.test.ts` tests: a supervisor returning `inject` lands its text in the next request's messages, and `inject` + `andThen: compact` triggers compaction with the supervisor's prompt (and returns early, skipping the max_tokens continue-prompt) while the text is still queued on the next turn's prefix. Mid-turn (`executeTools`) `inject` uses the same placeholder queueing as the end-turn path; real per-request injection is still stage 4.
   - **`annotation?: unknown` deleted.** Nothing read or wrote it; the annotation lands with stage 4's per-request injection, which is where its concrete type will be known.
   - **The `tool_use`-with-idle-runner state is modelled instead of faked.** `TurnActivity` gains `{type: "awaiting_tools"; activeTools}` — the runner has handed off and its `requested` list is gone, so `phase` no longer fabricates `{requested: [], truncated: false}`. Covered by a test.
