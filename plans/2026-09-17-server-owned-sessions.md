...
- Review follow-ups (stage 4 code review):
  - [x] `ScriptInvocation.status` became `state`, a discriminated union (`running | done | error(error) | aborted`), so a failed invocation must carry its reason; `handleChildExit` now records "script process exited before completing" instead of a reasonless error. Exported as `ScriptInvocationState`; views and tests read `inv.state.type`.
  - [x] `ScriptThreadResult` carries the structured yield (`value: unknown`). `toScriptResult` no longer stringifies and `resolveThread` no longer JSON.parses with a silent fallback; the SDK `Result` is sent as-is and the view stringifies for display.
  - [x] `ScriptSandboxRoot.toggle` is required (its only producer always supplied it).
  - [x] IPC input is validated at the boundary: `parseScriptToMagenta(raw): ScriptToMagenta | undefined` replaces the `raw as ScriptToMagenta` casts in both registration capture and the invocation message handler; malformed messages are logged and dropped.
  - [x] `normalizeThreadRequest` turns the all-optional `ThreadOptions` bag into one precise branded `ScriptThreadRequest`, so `createScriptThread` spreads a single object instead of five conditional spreads with brand casts.
  - [x] `ScriptController` registers named handlers and gained `dispose()` (called from `Magenta.destroy`); it drops `expandedThreads` for a removed invocation's threads, tracked in `invocationThreads` and mirrored on `invocationChanged` while the record still exists.
  - [x] `ScriptManager.dispose()` also clears the catalog, so a disposed manager renders and discovers nothing.
  - [x] New tests (`node/nvimclient/scripts/script-manager.test.ts`): toggling an invocation's sandbox with `t` auto-approves a spawned thread's pending permission and renders `SANDBOX OFF`; `dispose()` kills a running invocation's child, makes `startScript` throw, and leaves `discover()` a no-op.
- Decisions/deviations:
...
- Review follow-ups (stage 5 code review):
  - [x] `Magenta.destroy()` disposes the session even when script teardown rejects: the scripts rejection is caught and logged before chaining `session.dispose()`. New test `a failing script disposal still disposes the session` (`node/nvimclient/scripts/script-manager.test.ts`) stubs a rejecting `scripts.dispose` and asserts threads still settle and the session rejects new work.
  - [x] New test `a disposed view observes no further session events` (`node/nvimclient/chat/chat-view-adapter.test.ts`) pins `Chat.dispose()` detachment: after dispose, creating and deleting session threads leaves the view cache empty.
  - [x] `Chat`'s second constructor parameter is destructured as `{ session, host }` in the signature, so the `session.session` read is gone.
- Decisions/deviations:
