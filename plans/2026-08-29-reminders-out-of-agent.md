...
Review follow-ups (stage 4) — the `onBeforeContinuation` refactor, previously left uncommitted, was landed with the reviewer's findings addressed:

- `OnBeforeContinuation` now returns a `ContinuationDecision` discriminated union (`{ type: "continue" } | { type: "suspend"; reason: SuspendReason }`), and `TurnResult`'s `suspended` variant carries the optional `reason`, so the supervisor's suspension travels back on the result instead of through a mutable field. `Agent.suspendReason` shrank to `pendingYield` (the tool executor's yield is the only out-of-band case left — the runner has no notion of yield values). Both runners default a missing hook to `{ type: "continue" }` and branch on `decision.type`.
- `applyBeforeRequestActions(context, mode)` is split: `composeBeforeRequest(context)` returns `ComposedBeforeRequest` (`injections` + `submissions`, or a suspension) and each caller places the content itself — `submit` prefixes it to the opening request, `applyStopHooks` prepends to the next turn, `onBeforeContinuation` appends it (coalesced) to the tool-result message. `BeforeRequestResult` no longer carries a vestigial empty `injections`.
- New tests: `thread.test.ts` "delivers a mid-turn queued message on the tool_use continuation" (asserts the text is coalesced into the tool-result user message, after the `tool_result` block); `agent.test.ts` "reports the provider's stop reason on a continuation that carries tool results" (a `max_tokens` stop with tool_use blocks now reaches the hook as `continuation:max_tokens`, where the old code hardcoded `tool_use`); `runner-parity.test.ts` "onBeforeContinuation" — both runners return `{ type: "suspended", reason }` and issue no second request.
- Test churn from the coalescing: `thread.test.ts` (nvim) "getMessages correctly interleaves tool requests and responses" is 6 messages rather than 8 (block sequence unchanged); `system-reminders.test.ts`'s auto-respond test now asserts the reminder rides in the tool-result message after the `tool_result` block; one snapshot updated.
- Full suite: `npx tsc -b` and `npx biome check .` clean; `npx vitest run` 1593 passed, 2 skipped, 1 todo, with the pre-existing flaky `node/comments/comment-input.test.ts` failure reproduced on a stashed working tree at HEAD.
