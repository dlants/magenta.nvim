...
- Tests: full suite green (`npx vitest run`), plus `npx tsc -b` and `npx biome check .`.

Decisions / deviations:

- **`ContinuationRequest` gained `willRequest: boolean`.** `handleStopped` consults `onBeforeRequest` at *every* stop, including the one that ends the turn — that consult exists only so `AutoCompactSupervisor` can compact on an end_turn handoff, and three tests depend on it. But a stop that ends the turn issues no request, and letting the context trackers answer there drained (and *committed*) their updates into a message nothing was about to send. Two real failures showed it: a comment deleted during the async window of that consult was committed and landed in its own user message rather than with the user's next text, and the settle-path git shell-out delayed `settle`'s `onUpdate` past a thread switch, planting an unviewed-activity bell. The three context supervisors now return `none` when `kind === "continuation" && !willRequest`; `AutoCompactSupervisor` ignores the flag, so end_turn compaction is unchanged.
- **`handleStopped` decides the continuation before consulting.** `nextContinuation(stopReason)` returns the messages a stop will send (max_tokens prompt / drained queue / end-turn nudge) or `undefined`, which is what `willRequest` reports. It deliberately does *not* drain the queue: the drain still happens after the compaction check, so a compaction at a stop leaves the queued messages queued, exactly as before.
- **`applyBeforeRequestActions` mode `append` became `prefix`.** At a stop, injections go to `prependToNextTurn` rather than straight into the log, so they land in the *same* user message as whatever goes out next instead of a bare one of their own — restoring the pre-cutover message shape. The compaction branch still force-appends. `submit`'s early-settle now also checks `pendingTurnPrefix`, so a queued prefix cannot strand a contentless send.
- **`submit` consults the hooks before `prepareUserContent`.** The markdown-reminder scan reads `contextTracker.files[...].agentView`, which is what `FileContextSupervisor`'s update refreshes; building the reminder first read a stale view.
- `Agent.createToolContext().onToolApplied` no longer calls `contextManager.toolApplied` directly — the hook fan-out reaches `FileContextSupervisor.onToolApplied`, which is the only subscriber.
- `FileContextSupervisor.onSent` is a mutable public field and `GitSupervisor.gitTracker` is public: a fork has to clone the file supervisor (and read the source's git agent view) before the `NvimThread` that owns the display ledger exists.
- `NvimThread`'s third constructor parameter is now `preBuilt?: { core, fileSupervisor, commentStore }` (was `preBuiltCore?: Thread`). The fork path builds all three, since `Thread.clone`'s `AgentContext` needs `contextTracker` and `commentStore`. Comments are still not cloned — a forked root thread gets a fresh, empty store, as before.
- `NvimThread` keeps `supervisors` as the *behavioral* list (`chat.ts` still assigns it wholesale) and composes `[...contextSupervisors(), ...supervisors]`, which guarantees git then files then comments then behavioral then auto-compact without touching `chat.ts`.
- The core `Thread.destroy` test ("tears down the context manager") was deleted with the field; teardown is now covered by `FileContextSupervisor`'s own `destroy stops the poller` test, alongside a new `stays silent on a stop that issues no request` case.
- Two root tests were made timing-robust rather than adjusted in substance: the pending-expand drain and the reminder-header count both sample view state that arrives on the debounced re-render, and now poll for it.

Review follow-ups (stage 6 code review):

- **`ContinuationRequest.willRequest` is gone; a stop that issues no request is its own kind.** `RequestContextKind` is now `{kind:"submission"} | {kind:"continuation"; stopReason} | {kind:"turn-end"; stopReason}`, so "submission with willRequest: false" is not representable and the three context supervisors guard with a single `context.kind === "turn-end"`. `AutoCompactSupervisor` ignores the tag, so end_turn compaction is unchanged.
- New agent-level test `reports turn-end only for a stop that issues no request`: an `end_turn` stop with a queued async message reports `continuation`, the following one (nothing left to send) reports `turn-end`. The handoff test's third consult is now `turn-end` too.
- `GitSupervisor` and `CommentSupervisor` each gained a `stays silent on a stop that issues no request` test asserting `none`, no `onSent`, an uncommitted agent view / still-pending store, and that the update rides the *next* request.
- **`FileContextSupervisor.onSent` is `private readonly` again.** `FileContextSupervisor.clone` was replaced by `cloneContextManager(source, args)` in `context-manager.ts` (that is all it ever did), so the fork path clones the *manager* and `NvimThread` builds the supervisor itself, in one place, with its sink thunk. `preBuilt` now carries `contextManager` rather than `fileSupervisor`.
- **Root-ness lives in one field.** `commentController` / `commentStore` / `commentSupervisor` collapse into `comments?: ThreadComments` (`{store, supervisor, controller}`), built together or not at all; `isRootThread()` narrows to `RootNvimThread = NvimThread & { comments: ThreadComments }`, so one narrow reaches all three and the guard cannot lie. Callers use `thread.comments.controller` / `thread.comments?.store`.
- Not addressed (nit): `AgentContext.commentStore` stays an optional field rather than a thread-type-keyed variant. `AgentContext` is a wide, flat bag consumed by every tool context; splitting it on `threadType` is a larger refactor than this finding warrants and would not be local to the context-hooks work.
