...
Aggregation semantics: at each hook, `ThreadCore` consults **all** supervisors (in order) and combines their actions per-hook:
- `onEndTurnWithoutYield`: combine every returned `send-message` text (joined with blank lines); `none` if there are none.
- `onYield`: the first `accept`/`reject` (in supervisor order) wins; otherwise combine the `send-message` texts; `none` if there are none.
- `onHandoff`: if any supervisor returns `compact`, the result is `compact` with the returned `nextPrompt`s combined (joined with blank lines); `none` otherwise.

Because most supervisors only care about one or two hooks, make every `ThreadSupervisor` method **optional**; `ThreadCore` guards each call (`sup.onHandoff?.(...)`) and treats a missing method as `none`. There is no `onAbort` hook.
...
- `autoCompactPrompt: string` (default: a bundled "continue the task" sentence) — this is NOT a template override. The bundled `COMPACT_PROMPT_TEMPLATE` (with its magenta-owned `{{summary}}`/`{{chunk}}`/`{{status}}`/`{{next_prompt}}`/context blocks) is always used. `autoCompactPrompt` is the **`nextPrompt`** the auto-compacter supplies on behalf of the user (auto-compaction has no real user next prompt), which fills `{{next_prompt}}`. It is passed to `AutoCompactSupervisor` as its (required) `nextPrompt`; on `onHandoff` the supervisor returns `{ type: "compact", nextPrompt }`, and `ThreadCore.startCompaction` forwards it through the existing `CompactionManager` next-prompt path. There is no `compactPromptTemplate` override plumbing.
...
Per-thread overrides (magenta scripts): a script's `thread(prompt, yieldSchema, options)` call may set `autoCompactThreshold` and `autoCompactPrompt` per spawned thread. These override the global `MagentaOptions` defaults for that thread only. Precedence: per-thread option → global `MagentaOptions` → hard-coded default (`300000` / bundled default next-prompt sentence). The overrides flow `ThreadOptions` (`sdk/protocol.ts`) → `create-thread` IPC → `ScriptManager` → `chat.spawnScriptThread` → `createThreadWithContext`, where both are fed into the thread's `AutoCompactSupervisor` construction (`threshold` and `nextPrompt`). Threads spawned outside scripts simply omit the overrides and use the global defaults.
...
Code-review follow-ups (Stage 5):
- Added two integration tests in `node/chat/thread-compact.test.ts` covering the per-thread `autoCompactPrompt` override end-to-end (previously only the *global* option path and the per-thread *threshold* override were tested): (1) "script-spawned thread honors per-thread autoCompactPrompt override" spawns a script thread with a unique-marker template + low threshold, drives it to auto-compact, and asserts the compaction subagent request uses the per-thread template; (2) "script-spawned thread without prompt override falls back to the default template" asserts a script thread without the override compacts using the bundled `COMPACT_PROMPT_TEMPLATE` (marker absent, default text present). This exercises the load-bearing wiring `spawnScriptThread` → `createThreadWithContext` (chat.ts merges `autoCompactPrompt` into thread options) → `thread.ts` → `ThreadCoreContext.autoCompactPrompt` → `compactPromptTemplate`.
