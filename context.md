...
- **`runToolLoop`** (`node/server/src/tool-loop.ts`) runs one tool loop: provider requests alternating with tool batches. It returns a `ToolLoop` handle (`activity`, `promise` of `ToolLoopResult`) and is cancelled via the submission's AbortSignal; it is not an event emitter. Thread owns supervisor ordering and continuations beyond that loop.
